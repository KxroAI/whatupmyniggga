...
async def on_message(message):
    ...
    # --- Custom Text Replies (Relatively low volume, fine as is) ---
    if content == "hobie":
        # Add a small delay here too, as message sends can also be rate-limited
        ...
        await asyncio.sleep(0.3) # Add a delay before sending a message
        await message.channel.send(reply)
    elif content == "hello":
        await asyncio.sleep(0.3)
        await message.channel.send("hello, baby")
    
    # --- Automatic Reaction Logic (Needs careful throttling) ---
    
    # A list of channels where you want the default reaction
    auto_react_channels = [
        1225294057371074760, 1107600826664501258, 1107591404877791242,
        1368123462077513738
    ]
    
    # The minimum safe delay between any two API requests (reactions/messages)
    # 0.5 seconds is generally a very safe choice for reactions.
    SAFE_DELAY = 0.5 

    # Default Reaction (🎀)
    if message.channel.id in auto_react_channels:
        # 2. Add the safe delay *before* the reaction API call
        await asyncio.sleep(SAFE_DELAY) 
        try:
            await message.add_reaction("🎀")
        except discord.HTTPException:
             # This block will catch the 429 error if the library's internal handler fails,
             # or other common reaction errors (like forbidden permissions).
            print(f"Failed to add reaction 🎀 in channel {message.channel.id}. Rate-limited or permission issue.")
            pass # Fails silently and moves on

    if message.channel.id == 1107281584337461321:
        # 3. Add a second delay if this is another reaction on the same message
        await asyncio.sleep(SAFE_DELAY) 
        try:
            await message.add_reaction("<:1cy_heart:1258694384346468362>")
        except discord.HTTPException:
            print(f"Failed to add custom reaction in channel {message.channel.id}. Rate-limited or permission issue.")
            pass # Fails silently and moves on
